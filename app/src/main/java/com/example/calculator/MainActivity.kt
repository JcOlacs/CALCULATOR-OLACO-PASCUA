...
class MainActivity : AppCompatActivity() {
    ...
    private fun setListeners() {
        ...
        with(binding) {
            ...
            buttonAllClearBtn.setOnClickListener { onAllClearClicked() }
            buttonPlusMinusBtn.setOnClickListener { onPlusMinusClicked() }
    ...
    private fun onPercentageClicked() {
        if (inputValue2 == null) {
            ...
            val percentageOfValue1 = (getInputValue1() * getInputValue1()) / 100
    ...
    private fun onDecimalPointClicked() {
        if (equation.contains(DECIMAL)) return
        equation.append(DECIMAL)
        setInput()
        updateInputOnDisplay()
    }
    ...
    private fun clearDisplay() {
        with(binding) {
            textInput.text = getFormatedDisplayValue(value = getInputValue1())
            textEquation.text = null
        }
    }

    private fun updateResultOnDisplay(isPercentage: Boolean = false) {
        binding.textInput.text = getFormatedDisplayValue(value = result)
        var input2Text = getFormatedDisplayValue(value = getInputValue2())

        if(isPercentage) input2Text = "$input2Text${getString(R.string.percentage)}"
        binding.textEquation.text = String.format(
            "%s %s %s",
            getFormatedDisplayValue(value = getInputValue1()),
            getOperatorSymbol(),
            input2Text
        )
    }
    ...
    private fun getFormatedDisplayValue(value: Double?): String? {
